...
program description:
Create a sales record that loops through a menu and 
gives the user the option to name the file, then add, view sales records, and exit.
Data must persist
 ...
 */
   public static void main(String [] args) 
      ...
      output = new PrintStream(salesRecord);
      // records old information again on overwritten file
      for(int i = 0; i< count; i++){
    	  output.println(names[i] + " " + sales[i]);
      }
      
      //Loops through menu and then add/view methods.
      while(loop != 0){
    	  loop = menu();
    	  if(loop == 2){
    		  viewSale( names, sales, count);
...
/*
 * View String takes in both arrays and the counter for them and uses them to create
 * a report of all the people and their contributions proportional to the total.
 * 
 * */
   private static void viewSale( String[] names, double[] sales, int count) {
	   int totalSales = 0;
 ...
 * */
   private static void addSale(){
	   ...
	   //catching wrong input. Needs to be a double.
	   try{
